...
% conditions in radian 
conditions = [0 pi/6 2*pi/6 3*pi/6 4*pi/6 5*pi/6 pi];
conditionNames={'0 vs 180' '30 vs -150' '60 vs -120' '90 vs -90' '120 vs -60' '150 vs -30' '180 vs 0'};  
...
%             PolyshapeUnion=intersect(polyTrajatory,PolyshapGreenPixelInitial);
        ...
        while Insidestart == 0
            ...
            Screen('DrawDots', windowPtr, [xU(1) yU(1)], Thickness, white, [], 2);
            ...
            if Insidestart == 1
                Screen('DrawDots', windowPtr, [xU(1) yU(1)], Thickness, green, [], 2);
                % flip to screen
                vbl  = Screen('Flip', windowPtr, vbl + (waitframes -0.5) * ifi);
