...
xlim([0.5 4.5]);
ylabel('H','FontSize',17); ylim([0.4 1]);
set(gcf,'color','w'); % set background white for copying in ubuntu
...
for i=1:3
    ...
    elseif i==1 | i==3;
        ...
        ylabel('\rho','FontSize',20,'FontWeight','bold');xlabel('lag (L-leading <- 0 -> R-leading)','FontSize',17);ylim([-0.1 0.8])
    end    
    hold off;
    end
    yline(0,'color',[1 0.8 0.2]);xline(0,'color',[1 0.8 0.2]);
...
%      Corr4x5=...
%     corr( ...
%     LR_all_chan{(Inds4_LR(synind(t,subj),2)),f},...
%     LR_all_chan{(Inds4_LR(36+synind(t,subj),3)),f}...
%     );   
    else s==2 | s==3;
     Corr4x5=...
    corr( ...
