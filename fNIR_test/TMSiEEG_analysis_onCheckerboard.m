...
% examine
for i=1:numTrials % i=numTrials
    plot(TrialTime,reRef_data(:,:,i));pause(1); % with the artifacts
end 
...
% baselinesamps = 1:round(3*sr); % use the first 2 s as baseline
...
for u=[1 2]; % sp=1:2
    ...
    [pow,freqs,df] = allspectra(basenormalized_trial(win,:,indtemp),sr,maxfreq);
...
for u=[1 2]; % sp=1:2
    ...
    for chan=1:32
        ...
        set(gca,'ydir','normal');
        hold on;xline(6500,'m','linewidth',2);hold off;
        if any(goodchans(:) == chan) % Display good chan in green
            title([labels{chan}],'Color','green');
        else
            title([labels{chan}],'Color','red'); % Display bad chan in red
