...
y      = y + steplength/2 %  + yCenter - steplength/2;% + (yCenter - max(y) + (max(y) - min(y))/2); 
Centroidy =steplength + steplength/2 % + yCenter - steplength/2; % + (yCenter - max(y) + (max(y) - min(y))/2); % offset circle center
