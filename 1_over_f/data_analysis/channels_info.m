...
sph_phi = extractfield(chaninfo,'sph_phi');
sph_radius = extractfield(chaninfo,'sph_radius');
X = extractfield(chaninfo,'X');
...
% Yy=radius.*sin(Th);
% 
% Xx=sph_radius.*cos(sph_theta);
% Yy=sph_radius.*sin(sph_theta);
% 
% Xx=log(sph_phi).*cos(sph_theta);
% Yy=log(sph_phi).*sin(sph_theta);

% figure('units','normalized','outerposition',[0 0 1 1]);
% figure;
% plot(Xx,Yy,'r.','MarkerSize',25);
% text(Xx,Yy,labels,'VerticalAlignment','bottom','HorizontalAlignment','right');

% rotation matrix
theta_rota = 90; % to rotate 90 counterclockwise
R = [cosd(theta_rota) -sind(theta_rota); sind(theta_rota) cosd(theta_rota)];
pos_before=[Xx;Yy];
pos=zeros(2,32); % pos after
for i=1:32
    pos(:,i)=R*pos_before(:,i);
end
figure;
...
% move to origin
halfx=(max(pos(:,1))-min(pos(:,1)))/2;
halfy=(max(pos(:,2))-min(pos(:,2)))/2;
pos(:,1)=pos(:,1)-halfx;
pos(:,2)=pos(:,2)-halfy;
figure;
plot(pos(:,1),pos(:,2),'r.','MarkerSize',25);
