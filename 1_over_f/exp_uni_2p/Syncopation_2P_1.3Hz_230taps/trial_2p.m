...
%% trial loop
while (n < numFrames) & (n < MaxFrames) & (tapsRecordedL < MaxTaps ) & (tapsRecordedR < MaxTaps ) 
    % either one reach 600 taps / Maxframes of recording / MaxTaps 
    % have to manually advance by experimentor anyway
    ...
    % for experimentor 
    DrawFormattedText2([ConditionInstructions{condiSeq(conditionSelected)} ' -- trial ' num2str(t)],'win',windowPtr,...
    'sx','center','sy', screenYpixels*0.8,'xalign','center','yalign','top','baseColor',color);
    % for player L
    if condiSeq(conditionSelected) == 1;
        DrawFormattedText2(ConditionInstructionsL{condiSeq(conditionSelected)},'win',windowPtr,...
    ...
    % for player R
    if condiSeq(conditionSelected) == 1;
        DrawFormattedText2(ConditionInstructionsR{condiSeq(conditionSelected)},'win',windowPtr,...
...
%     'sx',xCenter+screenYpixels/4,'sy',yCenter-screenYpixels/15,'xalign','center','yalign','top','baseColor',color);
    ...
    if (condiSeq(conditionSelected) == 2) ;
        if any(ShowframesL(:) == n); 
            Screen('DrawDots', windowPtr, [xCenter-screenXpixels/3;yCenter], screenYpixels/20-2, red, [0 0], 2); % Left monitor
            Screen('FillRect', windowPtr, white, LeftUpperSquare); % photocell
            Screen('FillRect', windowPtr, white, RightUpperSquare); % photocell
    ...
    if (condiSeq(conditionSelected) == 3) ;
        if any(ShowframesR(:) == n); 
            Screen('DrawDots', windowPtr, [xCenter+screenXpixels/3;yCenter], screenYpixels/20-2, blue, [0 0], 2); % Right monitor
            Screen('FillRect', windowPtr, white, LeftUpperSquare); % photocell
            Screen('FillRect', windowPtr, white, RightUpperSquare); % photocell
    ...
    % show the other's resampled recording (only the first 30 taps)
    if (condiSeq(conditionSelected) == 4) ;
        if any(ShowframesR(1:30) == n); % Pace Left subject 
            % show R's realtime feedbacks after ShowframesR(30) = 1103 frames
            Screen('DrawDots', windowPtr, [xCenter-screenXpixels/3;yCenter], screenYpixels/20-2, blue, [0 0], 2); % Left monitor
            Screen('FillRect', windowPtr, white, LeftUpperSquare); % photocell
            Screen('FillRect', windowPtr, white, RightUpperSquare); % photocell
        end
        if any([ShowframesL(1:30)+NumFramesInterval13Hz/2] == n); % Pace right subject get started
            % show L's realtime feedbacks after ShowframesL(30)+NumFramesInterval13Hz/2=1122 frames
...
%             Screen('FillRect', windowPtr, white, RightUpperSquare); % photocell
    ...
    if pressedL1 ==1 %| pressedL2 == 1 % at least one key press detected in the frist two events of the previous buffer
        % if RBkeyL1 == 3 %| RBkeyL2 == 3 % confirm it is the middle key on RB740
            ...
            if RBkeyL1==6;
                % show warning yellow dot if last key was pressed
            ...
            % show feedback photocell to the other side
            ...
            if (condiSeq(conditionSelected) == 2); % show feedback to other
                Screen('DrawDots', windowPtr, [xCenter+screenXpixels/3;yCenter], screenYpixels/20-2, red, [0 0], 2); % Right monitor
            end
            if (condiSeq(conditionSelected) == 4) & (n > ShowframesR(30)); % 1103 % show feedback to other
                Screen('DrawDots', windowPtr, [xCenter+screenXpixels/3;yCenter], screenYpixels/20-2, red, [0 0], 2); % Right monitor
            end
    ...
    if pressedR1 ==1; %| pressedR2 == 1  % at least one key press detected in the frist two events of the previous buffer
        % if RBkeyR1 == 3 %| RBkeyR2 == 3  % confirm it is the middle key on RB740
            ...
            if RBkeyR1==6;
                % Show warning yellow dot instead if subject press the last key
            ...
            % show feedback to the other side
            ...
            if (condiSeq(conditionSelected) == 4) & (n > ShowframesL(30)+NumFramesInterval13Hz/2); %1122
                Screen('DrawDots', windowPtr, [xCenter-screenXpixels/3;yCenter], screenYpixels/20-2, blue, [0 0], 2); % Left monitor
            end
