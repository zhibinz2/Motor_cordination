...
for t=1:2 % type of session (synch/o)
    ...
    imagesc(delta_corr);grid on; grid minor;
    colorbar;
    clim([cmin cmax]);


    imagesc(corr([1:32],[1:32]));colorbar;
    imagesc(corr(repmat([1:32],10,1),repmat([1:32],10,1)));colorbar;
    imagesc(corr([repmat([1:7],10,1) rand(10,25)],[repmat([1:7],10,1) rand(10,25)]));colorbar;


    figure;
    diag(32)
    topoplot(diag(delta_corr),channels);
    colorbar;
    clim([cmin cmax]);
