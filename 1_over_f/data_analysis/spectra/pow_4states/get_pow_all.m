...
corr_all=nan(12,2,12,51,32,32); % session x L/R X trials x pow [51 freq x 32 chan x 32 chan]
...
for s=1:numSes
    ...
    for sub=1:2 % R/L
        ...
        % compute pow
        [pow_all(s,sub,t,:,:),~,~,corr_all(s,sub,t,:,:,:),ampcorr_all(s,sub,t,:,:,:),~] = allspectra1(epoches,sr,maxfreq);
        end
    end
end
