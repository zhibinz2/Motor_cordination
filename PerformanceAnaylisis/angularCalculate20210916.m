...
CenterXL = CentroidL(1); % steplength + steplength/2; 
CenterYL = CentroidL(2); % steplength + (yCenter - max(y) + (max(y) - min(y))/2); 
CenterXR = CentroidR(1); % steplength + screenXpixels/2 + steplength/6; 
CenterYR = CentroidR(2); % steplength + (yCenter - max(y) + (max(y) - min(y))/2); 
...
%% find the route start from 0 to pi (condition A2&A2)
...
%% Vector of the start side 0*pi (condition A2)
...
for u=1:length(UniCondi)
    ...
    %   plot(TracexL,TraceyL,'-r',TracexR,TraceyR,'-b');
        ...
        % Temperal
        DiffLR= AngR-AngL;
        rmseAng=mean(DiffLR); % in radiaus
