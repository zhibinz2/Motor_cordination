...
for i=1:4
    subplot(2,4,i);
    plot(-10:1:10,mean(squeeze(mean(Xcorr10Lag(1:2:11,sorted4inds(i,:),:),2)),1),'k');
    title(condi4names{i},'color',condicolors(i,:));
    subtitle('xcorr synch');
    ylabel('\rho(k)');xlabel('lag');ylim([-0.1 0.8])
    yline(0,'color',[1 0.8 0.2]);xline(0,'color',[1 0.8 0.2]);
    subplot(2,4,4+i);
    plot(-10:1:10,mean(squeeze(mean(Xcorr10Lag(2:2:12,sorted4inds(i,:),:),2)),1),'k');
    title(condi4names{i},'color',condicolors(i,:));
    subtitle('xcorr synco');
    ylabel('\rho(k)');xlabel('lag');ylim([-0.1 0.8])
    yline(0,'color',[1 0.8 0.2]);xline(0,'color',[1 0.8 0.2]);
end
suptitle('mean xcorr for 10 lags from all sessions (unmatched int) ^{ *PLOT9}')

% combine synch and synco of Xcorr10Lag in 3 directions
Xcorr10Lag; % sorted order
direction3names={'uncouple','uni-directional','mutual'};
...
sgtitle('mean xcorr sum for 10 lags in all sessions (unmatched int) ^{ *PLOT7}');
