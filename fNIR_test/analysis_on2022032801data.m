...
% end
% % examine every trial
% figure;plot(normalized_trials(:,:,20));
% figure;
% for i=1:72
...
%******************************************************************
% Plot before organization into trials
...
Ylim=1.5;Xlim=[100 300];
figure('units','normalized','outerposition',[0 0 1 0.6]);
subplot(2,1,1);% left channels
% plot(time,filtered_data(:,1:2:21),'r');hold on;
% plot(time,filtered_data(:,2:2:22),'b');
...
title('Left occ. cortex');hold off;ylim([-Ylim Ylim]);xlim(Xlim);
subplot(2,1,2);% right channels
% plot(time,filtered_data(:,23:2:43),'r');hold on;
% plot(time,filtered_data(:,24:2:44),'b');
...
title('Right occ. cortex');hold off;ylim([-Ylim Ylim]);xlim(Xlim);
