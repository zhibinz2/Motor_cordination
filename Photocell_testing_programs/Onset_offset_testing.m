cd /home/hnl/Documents/GitHub/Motor_cordination/Photocell_testing_programs
...
% Hz=input('enter the frequency:'); % 0.5Hz, once every 2 seconds

% set the duration of the photocell
duration=input('enter the duration in second:'); % 2, show photocell on for 2 s
waitframes=1;
...
numTrials=60;
% number of blocks
numBlock=10; % 3 blocks per session,6 subject
% total trial number
...
%     [windowPt
% scr, windowRect] = PsychImaging('Openwindow', screenNumber, black, [0 0 600 400]); 
    ...
    % set Priority once at the start of a script after 10setting up onscreen window.
...
%     total number of frames per trial
    ...
    numFrames=round(duration/ifi/waitframes); % duration= 2 seconds per trial
...
%     numFrames=round((1/ifi)*100); % 100 seconds 
    ...
    instructionStart=['OK. Press any key to start!']; % Tell subject to open eye and start
...
%             vbl = Screen('Flip', windowPtr);
            ...
            % Show photocell only in odd trials
            if mod(t,2) == 1;
                n=1;
    ...
    %                     Screen('FillRect', windowPtr, white, RightSide);
                        ...
                        % Flash only upper two corners 
                        Screen('FillRect', windowPtr, white, LeftUpperSquare); % event type = 1200001
                        Screen('FillRect', windowPtr, white, RightUpperSquare);  % event type = 1200001
    
                        % Flash only botton two corners
                        Screen('FillRect', windowPtr, white, LeftBottomSquare);
                        Screen('FillRect', windowPtr, white, RightBottomSquare);
    ...
    %                 Screen('Flip', windowPtr);
            ...
            % Show black screen during even trials
            if mod(t,2)==0;
                n=1;
                while n <  numFrames 
                    % If esc is press, break out of the while loop and close the screen
