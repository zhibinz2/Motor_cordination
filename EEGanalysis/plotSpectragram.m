...
for u=1:length(UniCondi);
    ...
    basedlinecorrected_ERP=mean(baselinecorrected_data_trial(1501:2500,1:128,HighInd),3);
    ...
    subplot(3,2,1);
    imagesc(abs(cnorm(:,:,9))); % F3/F4 9/13  
    colormap jet; colorbar ; %caxis% ([0 0.15]);% caxis 
    xlabel('time(ms)'); ylabel('frequencies(Hz)'); set(gca,'ydir','normal');
    title([AllchanNames{9}]);

    subplot(3,2,2);
    imagesc(abs(cnorm(:,:,13))); % F3/F4 9/13  
    colormap jet; colorbar ; %caxis% ([0 0.15]);
    xlabel('time(ms)'); ylabel('frequencies(Hz)'); set(gca,'ydir','normal');
    title([AllchanNames{13}]);

    subplot(3,2,3);
    imagesc(abs(cnorm(:,:,19))); % FC3/FC4 19/23 
    colormap jet; colorbar ; %caxis% ([0 0.15]); % caxis
    xlabel('time(ms)'); ylabel('frequencies(Hz)'); set(gca,'ydir','normal');
    title([AllchanNames{19}]);

    subplot(3,2,4);
    imagesc(abs(cnorm(:,:,23))); % FC3/FC4 19/23 
    colormap jet; colorbar ; %caxis% ([0 0.15]);
    xlabel('time(ms)'); ylabel('frequencies(Hz)'); set(gca,'ydir','normal');
    title([AllchanNames{23}]);

    subplot(3,2,5);
    imagesc(abs(cnorm(:,:,28))); % C3 
    colormap jet; colorbar ; %caxis% ([0 0.15]);
    xlabel('time(ms)'); ylabel('frequencies(Hz)'); set(gca,'ydir','normal');
    title([AllchanNames{28}]);

    subplot(3,2,6);
    imagesc(abs(cnorm(:,:,32))); % C4 
    colormap jet; colorbar ; %caxis% ([0 0.15]);
    xlabel('time(ms)'); ylabel('frequencies(Hz)'); set(gca,'ydir','normal');
