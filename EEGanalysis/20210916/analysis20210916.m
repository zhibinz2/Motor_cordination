...
%% load behaviral data
clear;
cd('/home/zhibin/Documents/Acquisition/zhibin20210916')
load('matlab20210916.mat');
...
% load EEG all as one
EEG=loadcurry([pwd '/bimanual_halfcir_zhibin_sr1k_20210916.cdt']);
dataEEG=EEG.data;
datatimes=EEG.times;
Fs=EEG.srate;
% plot(datatimes,dataEEG);

%% photocell processing to find trial start and end 
% plot photocell analog signal
analog1=133; 
analog1data=dataEEG(analog1,:);
% plot(datatimes,analog1data);
% might need to mannually cut off the start and end of block

% analog2=134; 
% analog2data=dataEEG(analog2,:);
% plot(datatimes,analog2data);
% % weird start and end of block signal
...
% % Just to check out
CondiData;
figure;plot(CondiData);yticks([1 2 3 4]);
