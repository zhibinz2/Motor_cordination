function DrawArclowLoop(samplingRate,trialLength)
...
%     HideCursor;
    ...
    while GetSecs<stop
        loopStart = GetSecs;
         ...
         % Draw a green dot where the mouse cursor is
         Screen('DrawDots', window, [xM yM], 10, green, [], 2);
         
         
        %This loop runs for ifi or until it successfully retrieves some data from the queue
        while 1  %Note this loop MUST be broken manually, as 'while 1' always returns TRUE
            pkt = WinTabMex(5);
            
            %This check breaks the loop if data is recovered from the queue before ifi is up
            if ~isempty(pkt)
                break
            end
            
            %This check breaks the loop after ifi if pkt was always empty
            if GetSecs>(loopStart+ifi)
        ...
        %Waits to end of ifi if need be
        if GetSecs<(loopStart+ifi)
            WaitSecs('UntilTime', loopStart+ifi);
        end
    end
